...
import { getPhase6AStagedDeletionScaffold } from './staged_deletion.scaffold';

function testStagedDeletionScaffoldMetadata() {
  const scaffold = getPhase6AStagedDeletionScaffold();

  assert.equal(scaffold.phase, '6A');
  assert.equal(scaffold.ticket_id, 'P6A-FFET-007');
  assert.equal(scaffold.seed_id, 'seed_6a_staged_deletion');
...
function run() {
  testStagedDeletionScaffoldMetadata();
  console.log('P6A-FFET-007 staged deletion scaffold test passed.');
}
