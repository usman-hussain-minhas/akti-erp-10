...
import { getPhase6ACustomFieldsScaffold } from './custom_fields.scaffold';

function testCustomFieldsScaffoldMetadata() {
  const scaffold = getPhase6ACustomFieldsScaffold();

  assert.equal(scaffold.phase, '6A');
  assert.equal(scaffold.ticket_id, 'P6A-FFET-049');
  assert.equal(scaffold.seed_id, 'seed_6a_custom_fields');
...
function run() {
  testCustomFieldsScaffoldMetadata();
  console.log('P6A-FFET-049 custom fields scaffold test passed.');
}
