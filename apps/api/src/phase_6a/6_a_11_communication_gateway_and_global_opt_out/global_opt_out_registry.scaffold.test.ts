...
import { getPhase6AGlobalOptOutRegistryScaffold } from './global_opt_out_registry.scaffold';

function testGlobalOptOutRegistryScaffoldMetadata() {
  const scaffold = getPhase6AGlobalOptOutRegistryScaffold();

  assert.equal(scaffold.phase, '6A');
  assert.equal(scaffold.ticket_id, 'P6A-FFET-035');
  assert.equal(scaffold.seed_id, 'seed_6a_global_opt_out_registry');
...
function run() {
  testGlobalOptOutRegistryScaffoldMetadata();
  console.log('P6A-FFET-035 global opt out registry scaffold test passed.');
}
