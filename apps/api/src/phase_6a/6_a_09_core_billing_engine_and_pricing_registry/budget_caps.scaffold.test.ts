...
import { getPhase6ABudgetCapsScaffold } from './budget_caps.scaffold';

function testBudgetCapsScaffoldMetadata() {
  const scaffold = getPhase6ABudgetCapsScaffold();

  assert.equal(scaffold.phase, '6A');
  assert.equal(scaffold.ticket_id, 'P6A-FFET-022');
  assert.equal(scaffold.seed_id, 'seed_6a_budget_caps');
...
function run() {
  testBudgetCapsScaffoldMetadata();
  console.log('P6A-FFET-022 budget caps scaffold test passed.');
}
