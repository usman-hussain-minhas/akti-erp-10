...
import { getPhase6AIndustrySelectionScaffold } from './industry_selection.scaffold';

function testIndustrySelectionScaffoldMetadata() {
  const scaffold = getPhase6AIndustrySelectionScaffold();

  assert.equal(scaffold.phase, '6A');
  assert.equal(scaffold.ticket_id, 'P6A-FFET-069');
  assert.equal(scaffold.seed_id, 'seed_6a_industry_selection');
...
function run() {
  testIndustrySelectionScaffoldMetadata();
  console.log('P6A-FFET-069 industry selection scaffold test passed.');
}
