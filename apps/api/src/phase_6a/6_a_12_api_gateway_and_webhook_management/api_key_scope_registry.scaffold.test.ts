...
import { getPhase6AApiKeyScopeRegistryScaffold } from './api_key_scope_registry.scaffold';

function testApiKeyScopeRegistryScaffoldMetadata() {
  const scaffold = getPhase6AApiKeyScopeRegistryScaffold();

  assert.equal(scaffold.phase, '6A');
  assert.equal(scaffold.ticket_id, 'P6A-FFET-039');
  assert.equal(scaffold.seed_id, 'seed_6a_api_key_scope_registry');
...
function run() {
  testApiKeyScopeRegistryScaffoldMetadata();
  console.log('P6A-FFET-039 api key scope registry scaffold test passed.');
}
