...
import { getPhase6ADeliveryRejectionLogsScaffold } from './delivery_rejection_logs.scaffold';

function testDeliveryRejectionLogsScaffoldMetadata() {
  const scaffold = getPhase6ADeliveryRejectionLogsScaffold();

  assert.equal(scaffold.phase, '6A');
  assert.equal(scaffold.ticket_id, 'P6A-FFET-044');
  assert.equal(scaffold.seed_id, 'seed_6a_delivery_rejection_logs');
...
function run() {
  testDeliveryRejectionLogsScaffoldMetadata();
  console.log('P6A-FFET-044 delivery rejection logs scaffold test passed.');
}
