...
import { getPhase6AFoundryRuntimeAuthorityScaffold } from './foundry_runtime_authority.scaffold';

function testFoundryRuntimeAuthorityScaffoldMetadata() {
  const scaffold = getPhase6AFoundryRuntimeAuthorityScaffold();

  assert.equal(scaffold.phase, '6A');
  assert.equal(scaffold.ticket_id, 'P6A-FFET-024');
  assert.equal(scaffold.seed_id, 'seed_6a_foundry_runtime_authority');
...
function run() {
  testFoundryRuntimeAuthorityScaffoldMetadata();
  console.log('P6A-FFET-024 foundry runtime authority scaffold test passed.');
}
