...
import { getPhase6APricingReferenceRegistrationScaffold } from './pricing_reference_registration.scaffold';

function testPricingReferenceRegistrationScaffoldMetadata() {
  const scaffold = getPhase6APricingReferenceRegistrationScaffold();

  assert.equal(scaffold.phase, '6A');
  assert.equal(scaffold.ticket_id, 'P6A-FFET-031');
  assert.equal(scaffold.seed_id, 'seed_6a_pricing_reference_registration');
...
function run() {
  testPricingReferenceRegistrationScaffoldMetadata();
  console.log('P6A-FFET-031 pricing reference registration scaffold test passed.');
}
