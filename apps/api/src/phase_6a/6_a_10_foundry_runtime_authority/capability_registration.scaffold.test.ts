...
import { getPhase6ACapabilityRegistrationScaffold } from './capability_registration.scaffold';

function testCapabilityRegistrationScaffoldMetadata() {
  const scaffold = getPhase6ACapabilityRegistrationScaffold();

  assert.equal(scaffold.phase, '6A');
  assert.equal(scaffold.ticket_id, 'P6A-FFET-030');
  assert.equal(scaffold.seed_id, 'seed_6a_capability_registration');
...
function run() {
  testCapabilityRegistrationScaffoldMetadata();
  console.log('P6A-FFET-030 capability registration scaffold test passed.');
}
