...
import { getPhase6ADeactivationDependencyBlockingScaffold } from './deactivation_dependency_blocking.scaffold';

function testDeactivationDependencyBlockingScaffoldMetadata() {
  const scaffold = getPhase6ADeactivationDependencyBlockingScaffold();

  assert.equal(scaffold.phase, '6A');
  assert.equal(scaffold.ticket_id, 'P6A-FFET-028');
  assert.equal(scaffold.seed_id, 'seed_6a_deactivation_dependency_blocking');
...
function run() {
  testDeactivationDependencyBlockingScaffoldMetadata();
  console.log('P6A-FFET-028 deactivation dependency blocking scaffold test passed.');
}
