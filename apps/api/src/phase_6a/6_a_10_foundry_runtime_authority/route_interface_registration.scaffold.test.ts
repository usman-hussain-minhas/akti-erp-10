...
import { getPhase6ARouteInterfaceRegistrationScaffold } from './route_interface_registration.scaffold';

function testRouteInterfaceRegistrationScaffoldMetadata() {
  const scaffold = getPhase6ARouteInterfaceRegistrationScaffold();

  assert.equal(scaffold.phase, '6A');
  assert.equal(scaffold.ticket_id, 'P6A-FFET-033');
  assert.equal(scaffold.seed_id, 'seed_6a_route_interface_registration');
...
function run() {
  testRouteInterfaceRegistrationScaffoldMetadata();
  console.log('P6A-FFET-033 route interface registration scaffold test passed.');
}
