...
import { Phase6BPerUnitPricingModelConfig } from './per_unit_pricing_model.config';

const config = new Phase6BPerUnitPricingModelConfig();

const baseInput = {
  organization_id: 'org_phase_6b',
  product_id: 'product_standard_plan',
  price_history_id: 'price_history_per_unit_2026_06',
  source_seed_id: 'seed_6b_02_per_unit_pricing_model' as const,
  unit_of_measure: 'seat',
  unit_amount_minor_units: 2500,
  currency_code: 'USD',
  effective_from: '2026-06-01T00:00:00.000Z',
  actor_user_id: 'user_pricing_admin',
  evidence_id: 'evidence_per_unit_2026_06',
  invoice_snapshot_required: true as const,
  independent_activation_requested: false,
};

const decision = config.validate(baseInput);
assert.equal(decision.seed_id, 'seed_6b_02_per_unit_pricing_model');
...
assert.deepEqual(decision.per_unit_price, {
  unit_of_measure: 'seat',
  unit_amount_minor_units: 2500,
  currency_code: 'USD',
});
assert.equal(decision.effective_window.effective_to, null);
assert.equal(decision.evidence.validation_event, 'PHASE_6B_PER_UNIT_PRICING_MODEL_CONFIG_VALIDATED');

const zeroPriceDecision = config.validate({
  ...baseInput,
  unit_amount_minor_units: 0,
});
assert.equal(zeroPriceDecision.per_unit_price.unit_amount_minor_units, 0);
...
console.log('P6B-FFET-008 per-unit pricing model config test passed.');
