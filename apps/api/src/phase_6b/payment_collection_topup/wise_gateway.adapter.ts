...
const CHANNELS: readonly WiseGatewayChannel[] = ['WISE_BALANCE_TRANSFER', 'BANK_TRANSFER', 'LOCAL_ACCOUNT_REFERENCE'] as const;
const ENVIRONMENTS: readonly WiseGatewayEnvironment[] = ['SANDBOX', 'PRODUCTION_DISABLED'] as const;
const CURRENCY_PATTERN = /^[A-Z]{3}$/;

function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(`${field} is required for Wise gateway.`);
  }
  return value.trim();
}
...
function requireTimestamp(value: string, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(`${field} must be a valid ISO-compatible timestamp for Wise gateway.`);
  }
  return normalized;
}
...
function normalizeCurrency(value: string): string {
  ...
  if (!CURRENCY_PATTERN.test(currency)) {
    throw new Error('currency_code must be a three-letter ISO-style code for Wise gateway.');
  }
  return currency;
}
...
function normalizeAmount(value: number): number {
  if (!Number.isInteger(value) || value <= 0) {
    throw new Error('amount_minor must be a positive integer for Wise gateway.');
  }
  return value;
}

function requireChannel(value: WiseGatewayChannel): WiseGatewayChannel {
  if (!CHANNELS.includes(value)) {
    throw new Error('channel is not supported for Wise gateway.');
  }
  return value;
}

function requireEnvironment(value: WiseGatewayEnvironment): WiseGatewayEnvironment {
  if (!ENVIRONMENTS.includes(value)) {
    throw new Error('provider_environment is not supported for Wise gateway.');
  }
  return value;
}
...
export function prepareWiseGatewayEnvelope(input: WiseGatewayPaymentInput): WiseGatewayEnvelopeReceipt {
  if (input.live_dispatch_requested === true) {
    throw new Error('Wise gateway FFET must not perform live provider dispatch.');
  }
  if (input.provider_callback_processing_requested === true) {
    throw new Error('Wise gateway FFET must not process provider callbacks.');
  }
  if (input.credential_material_present === true) {
    throw new Error('Wise gateway FFET must not handle credential material.');
  }
  if (input.payment_allocation_requested === true) {
    throw new Error('Wise gateway FFET must not perform payment allocation math.');
  }
  if (input.reconciliation_requested === true) {
    throw new Error('Wise gateway FFET must not perform reconciliation behavior.');
  }
  if (input.irreversible_action_requested === true) {
    throw new Error('Wise gateway FFET must not perform irreversible actions.');
  }
  ...
  return {
    seed_id: PHASE_6B_WISE_GATEWAY_SEED_ID,
    component_id: PHASE_6B_WISE_GATEWAY_COMPONENT_ID,
    event_name: WISE_GATEWAY_ENVELOPE_EVENT,
    ...
    provider_transaction_evidence_ref: `provider_transaction_evidence:WISE:${idempotencyKey}`,
