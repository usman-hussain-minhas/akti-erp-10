...
async function testBrandingReadSubstrateUsesOrganizationSettingLogoUrl() {
  ...
  const resolved = await service.resolveBrandingReadSubstrate('org-alpha', 'actor-alpha');
...
async function testBrandingReadSubstrateDefaultsWhenSettingIsAbsent() {
...
async function testBrandingReadSubstrateRejectsCrossTenantActorBeforeSettingRead() {
  const { service, state } = createService({
    brandingAssets: {
      logo_url: 'https://assets.example.test/logo.svg',
    },
  ...
  await assert.rejects(
    service.resolveBrandingReadSubstrate('org-alpha', 'actor-beta'),
    (error: unknown) => error instanceof BadRequestException,
  );
  assert.equal(state.calls.some((call) => call.fn === 'organizationSetting.findUnique'), false);
}

async function testBrandingReadSubstrateFailsClosedForUnsafeLogoUrl() {
  const { service } = createService({
    brandingAssets: {
  ...
  await assert.rejects(
    service.resolveBrandingReadSubstrate('org-alpha', 'actor-alpha'),
    (error: unknown) => error instanceof ConflictException,
  );
}

async function run() {
  await testBrandingReadSubstrateUsesOrganizationSettingLogoUrl();
  await testBrandingReadSubstrateDefaultsWhenSettingIsAbsent();
  await testBrandingReadSubstrateRejectsCrossTenantActorBeforeSettingRead();
  await testBrandingReadSubstrateFailsClosedForUnsafeLogoUrl();

  console.log('P5B1-006 branding read substrate tests passed.');
}
