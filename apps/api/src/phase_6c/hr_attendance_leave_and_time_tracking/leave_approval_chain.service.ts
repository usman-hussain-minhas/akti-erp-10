...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for leave_approval_chain scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for leave_approval_chain scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<LeaveApprovalChainScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluateLeaveApprovalChainScaffold(input: LeaveApprovalChainScaffoldInput): LeaveApprovalChainScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('leave_approval_chain scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('leave_approval_chain scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('leave_approval_chain scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<LeaveApprovalChainScaffoldReceipt, 'scaffold_evidence_digest'> = {
    seed_id: PHASE_6C_LEAVE_APPROVAL_CHAIN_SEED_ID,
    component_id: PHASE_6C_LEAVE_APPROVAL_CHAIN_COMPONENT_ID,
    component_slug: "hr_attendance_leave_and_time_tracking",
    model_name: "Phase6CLeaveApprovalChain",
    event_name: LEAVE_APPROVAL_CHAIN_SCAFFOLD_EVENT,
