...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for attendance_location_redaction scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for attendance_location_redaction scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<AttendanceLocationRedactionScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluateAttendanceLocationRedactionScaffold(input: AttendanceLocationRedactionScaffoldInput): AttendanceLocationRedactionScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('attendance_location_redaction scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('attendance_location_redaction scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('attendance_location_redaction scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<AttendanceLocationRedactionScaffoldReceipt, 'scaffold_evidence_digest'> = {
    seed_id: PHASE_6C_ATTENDANCE_LOCATION_REDACTION_SEED_ID,
    component_id: PHASE_6C_ATTENDANCE_LOCATION_REDACTION_COMPONENT_ID,
    component_slug: "hr_attendance_leave_and_time_tracking",
    model_name: "Phase6CAttendanceLocationRedaction",
    event_name: ATTENDANCE_LOCATION_REDACTION_SCAFFOLD_EVENT,
