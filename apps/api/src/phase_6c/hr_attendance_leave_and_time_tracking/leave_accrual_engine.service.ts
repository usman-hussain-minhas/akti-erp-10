...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for leave_accrual_engine scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for leave_accrual_engine scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<LeaveAccrualEngineScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluateLeaveAccrualEngineScaffold(input: LeaveAccrualEngineScaffoldInput): LeaveAccrualEngineScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('leave_accrual_engine scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('leave_accrual_engine scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('leave_accrual_engine scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<LeaveAccrualEngineScaffoldReceipt, 'scaffold_evidence_digest'> = {
    seed_id: PHASE_6C_LEAVE_ACCRUAL_ENGINE_SEED_ID,
    component_id: PHASE_6C_LEAVE_ACCRUAL_ENGINE_COMPONENT_ID,
    component_slug: "hr_attendance_leave_and_time_tracking",
    model_name: "Phase6CLeaveAccrualEngine",
    event_name: LEAVE_ACCRUAL_ENGINE_SCAFFOLD_EVENT,
