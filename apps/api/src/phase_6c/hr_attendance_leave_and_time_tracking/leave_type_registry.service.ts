...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for leave_type_registry scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for leave_type_registry scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<LeaveTypeRegistryScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluateLeaveTypeRegistryScaffold(input: LeaveTypeRegistryScaffoldInput): LeaveTypeRegistryScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('leave_type_registry scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('leave_type_registry scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('leave_type_registry scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<LeaveTypeRegistryScaffoldReceipt, 'scaffold_evidence_digest'> = {
    seed_id: PHASE_6C_LEAVE_TYPE_REGISTRY_SEED_ID,
    component_id: PHASE_6C_LEAVE_TYPE_REGISTRY_COMPONENT_ID,
    component_slug: "hr_attendance_leave_and_time_tracking",
    model_name: "Phase6CLeaveTypeRegistry",
    event_name: LEAVE_TYPE_REGISTRY_SCAFFOLD_EVENT,
