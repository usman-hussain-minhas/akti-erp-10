...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for leave_carryforward_expiry scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for leave_carryforward_expiry scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<LeaveCarryforwardExpiryScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluateLeaveCarryforwardExpiryScaffold(input: LeaveCarryforwardExpiryScaffoldInput): LeaveCarryforwardExpiryScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('leave_carryforward_expiry scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('leave_carryforward_expiry scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('leave_carryforward_expiry scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<LeaveCarryforwardExpiryScaffoldReceipt, 'scaffold_evidence_digest'> = {
    seed_id: PHASE_6C_LEAVE_CARRYFORWARD_EXPIRY_SEED_ID,
    component_id: PHASE_6C_LEAVE_CARRYFORWARD_EXPIRY_COMPONENT_ID,
    component_slug: "hr_attendance_leave_and_time_tracking",
    model_name: "Phase6CLeaveCarryforwardExpiry",
    event_name: LEAVE_CARRYFORWARD_EXPIRY_SCAFFOLD_EVENT,
