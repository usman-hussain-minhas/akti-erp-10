...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for kiosk_mode_checkin scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for kiosk_mode_checkin scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<KioskModeCheckinScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluateKioskModeCheckinScaffold(input: KioskModeCheckinScaffoldInput): KioskModeCheckinScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('kiosk_mode_checkin scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('kiosk_mode_checkin scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('kiosk_mode_checkin scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<KioskModeCheckinScaffoldReceipt, 'scaffold_evidence_digest'> = {
    seed_id: PHASE_6C_KIOSK_MODE_CHECKIN_SEED_ID,
    component_id: PHASE_6C_KIOSK_MODE_CHECKIN_COMPONENT_ID,
    component_slug: "events_check_in_and_post_event_service",
    model_name: "Phase6CKioskModeCheckin",
    event_name: KIOSK_MODE_CHECKIN_SCAFFOLD_EVENT,
    ...
    business_behavior_allowed: false,
    runtime_adapter_allowed: false,
    decision_refs: ["6C-EVENT-CHECK-004"],
