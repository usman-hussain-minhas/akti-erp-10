...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for offline_checkin_queue scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for offline_checkin_queue scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<OfflineCheckinQueueScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluateOfflineCheckinQueueScaffold(input: OfflineCheckinQueueScaffoldInput): OfflineCheckinQueueScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('offline_checkin_queue scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('offline_checkin_queue scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('offline_checkin_queue scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<OfflineCheckinQueueScaffoldReceipt, 'scaffold_evidence_digest'> = {
    seed_id: PHASE_6C_OFFLINE_CHECKIN_QUEUE_SEED_ID,
    component_id: PHASE_6C_OFFLINE_CHECKIN_QUEUE_COMPONENT_ID,
    component_slug: "events_check_in_and_post_event_service",
    model_name: "Phase6COfflineCheckinQueue",
    event_name: OFFLINE_CHECKIN_QUEUE_SCAFFOLD_EVENT,
    ...
    business_behavior_allowed: false,
    runtime_adapter_allowed: false,
    decision_refs: ["6C-EVENT-CHECK-008"],
