...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for task_record scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for task_record scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<TaskRecordScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluateTaskRecordScaffold(input: TaskRecordScaffoldInput): TaskRecordScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('task_record scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('task_record scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('task_record scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<TaskRecordScaffoldReceipt, 'scaffold_evidence_digest'> = {
    seed_id: PHASE_6C_TASK_RECORD_SEED_ID,
    component_id: PHASE_6C_TASK_RECORD_COMPONENT_ID,
    component_slug: "workspace_tasks_projects_documents_and_knowledge",
    model_name: "Phase6CTaskRecord",
    event_name: TASK_RECORD_SCAFFOLD_EVENT,
