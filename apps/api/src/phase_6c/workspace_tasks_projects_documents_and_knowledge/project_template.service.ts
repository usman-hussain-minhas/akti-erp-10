...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for project_template scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for project_template scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<ProjectTemplateScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluateProjectTemplateScaffold(input: ProjectTemplateScaffoldInput): ProjectTemplateScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('project_template scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('project_template scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('project_template scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<ProjectTemplateScaffoldReceipt, 'scaffold_evidence_digest'> = {
    seed_id: PHASE_6C_PROJECT_TEMPLATE_SEED_ID,
    component_id: PHASE_6C_PROJECT_TEMPLATE_COMPONENT_ID,
    component_slug: "workspace_tasks_projects_documents_and_knowledge",
    model_name: "Phase6CProjectTemplate",
    event_name: PROJECT_TEMPLATE_SCAFFOLD_EVENT,
    ...
    business_behavior_allowed: false,
    runtime_adapter_allowed: false,
    decision_refs: ["6C-WORK-TASK-012"],
