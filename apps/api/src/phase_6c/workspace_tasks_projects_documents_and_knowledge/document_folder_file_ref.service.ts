...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for document_folder_file_ref scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for document_folder_file_ref scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<DocumentFolderFileRefScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluateDocumentFolderFileRefScaffold(input: DocumentFolderFileRefScaffoldInput): DocumentFolderFileRefScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('document_folder_file_ref scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('document_folder_file_ref scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('document_folder_file_ref scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<DocumentFolderFileRefScaffoldReceipt, 'scaffold_evidence_digest'> = {
    seed_id: PHASE_6C_DOCUMENT_FOLDER_FILE_REF_SEED_ID,
    component_id: PHASE_6C_DOCUMENT_FOLDER_FILE_REF_COMPONENT_ID,
    component_slug: "workspace_tasks_projects_documents_and_knowledge",
    model_name: "Phase6CDocumentFolderFileRef",
    event_name: DOCUMENT_FOLDER_FILE_REF_SCAFFOLD_EVENT,
