...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for time_log_evidence scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for time_log_evidence scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<TimeLogEvidenceScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluateTimeLogEvidenceScaffold(input: TimeLogEvidenceScaffoldInput): TimeLogEvidenceScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('time_log_evidence scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('time_log_evidence scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('time_log_evidence scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<TimeLogEvidenceScaffoldReceipt, 'scaffold_evidence_digest'> = {
    seed_id: PHASE_6C_TIME_LOG_EVIDENCE_SEED_ID,
    component_id: PHASE_6C_TIME_LOG_EVIDENCE_COMPONENT_ID,
    component_slug: "workspace_tasks_projects_documents_and_knowledge",
    model_name: "Phase6CTimeLogEvidence",
    event_name: TIME_LOG_EVIDENCE_SCAFFOLD_EVENT,
