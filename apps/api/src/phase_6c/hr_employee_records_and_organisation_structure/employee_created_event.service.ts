...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for employee_created_event scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for employee_created_event scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<EmployeeCreatedEventScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluateEmployeeCreatedEventScaffold(input: EmployeeCreatedEventScaffoldInput): EmployeeCreatedEventScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('employee_created_event scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('employee_created_event scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('employee_created_event scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<EmployeeCreatedEventScaffoldReceipt, 'scaffold_evidence_digest'> = {
    seed_id: PHASE_6C_EMPLOYEE_CREATED_EVENT_SEED_ID,
    component_id: PHASE_6C_EMPLOYEE_CREATED_EVENT_COMPONENT_ID,
    component_slug: "hr_employee_records_and_organisation_structure",
    model_name: "Phase6CEmployeeCreatedEvent",
    event_name: EMPLOYEE_CREATED_EVENT_SCAFFOLD_EVENT,
