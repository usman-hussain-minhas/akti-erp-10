...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for compensation_metadata_payroll_evidence scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for compensation_metadata_payroll_evidence scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<CompensationMetadataPayrollEvidenceScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluateCompensationMetadataPayrollEvidenceScaffold(input: CompensationMetadataPayrollEvidenceScaffoldInput): CompensationMetadataPayrollEvidenceScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('compensation_metadata_payroll_evidence scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('compensation_metadata_payroll_evidence scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('compensation_metadata_payroll_evidence scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<CompensationMetadataPayrollEvidenceScaffoldReceipt, 'scaffold_evidence_digest'> = {
    ...
    component_slug: "hr_employee_records_and_organisation_structure",
    model_name: "Phase6CCompensationMetadataPayrollEvidence",
    event_name: COMPENSATION_METADATA_PAYROLL_EVIDENCE_SCAFFOLD_EVENT,
