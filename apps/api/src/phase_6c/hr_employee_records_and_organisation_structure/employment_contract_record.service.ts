...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for employment_contract_record scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for employment_contract_record scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<EmploymentContractRecordScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluateEmploymentContractRecordScaffold(input: EmploymentContractRecordScaffoldInput): EmploymentContractRecordScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('employment_contract_record scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('employment_contract_record scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('employment_contract_record scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<EmploymentContractRecordScaffoldReceipt, 'scaffold_evidence_digest'> = {
    seed_id: PHASE_6C_EMPLOYMENT_CONTRACT_RECORD_SEED_ID,
    component_id: PHASE_6C_EMPLOYMENT_CONTRACT_RECORD_COMPONENT_ID,
    component_slug: "hr_employee_records_and_organisation_structure",
    model_name: "Phase6CEmploymentContractRecord",
    event_name: EMPLOYMENT_CONTRACT_RECORD_SCAFFOLD_EVENT,
