...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for commission_payroll_batch_event scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for commission_payroll_batch_event scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<CommissionPayrollBatchEventScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluateCommissionPayrollBatchEventScaffold(input: CommissionPayrollBatchEventScaffoldInput): CommissionPayrollBatchEventScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('commission_payroll_batch_event scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('commission_payroll_batch_event scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('commission_payroll_batch_event scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<CommissionPayrollBatchEventScaffoldReceipt, 'scaffold_evidence_digest'> = {
    seed_id: PHASE_6C_COMMISSION_PAYROLL_BATCH_EVENT_SEED_ID,
    component_id: PHASE_6C_COMMISSION_PAYROLL_BATCH_EVENT_COMPONENT_ID,
    component_slug: "hr_performance_commission_policy_and_offboarding",
    model_name: "Phase6CCommissionPayrollBatchEvent",
    event_name: COMMISSION_PAYROLL_BATCH_EVENT_SCAFFOLD_EVENT,
