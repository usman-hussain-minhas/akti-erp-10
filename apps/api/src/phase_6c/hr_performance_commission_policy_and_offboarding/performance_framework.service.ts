...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for performance_framework scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for performance_framework scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<PerformanceFrameworkScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluatePerformanceFrameworkScaffold(input: PerformanceFrameworkScaffoldInput): PerformanceFrameworkScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('performance_framework scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('performance_framework scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('performance_framework scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<PerformanceFrameworkScaffoldReceipt, 'scaffold_evidence_digest'> = {
    seed_id: PHASE_6C_PERFORMANCE_FRAMEWORK_SEED_ID,
    component_id: PHASE_6C_PERFORMANCE_FRAMEWORK_COMPONENT_ID,
    component_slug: "hr_performance_commission_policy_and_offboarding",
    model_name: "Phase6CPerformanceFramework",
    event_name: PERFORMANCE_FRAMEWORK_SCAFFOLD_EVENT,
