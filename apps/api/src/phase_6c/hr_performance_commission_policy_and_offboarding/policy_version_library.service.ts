...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for policy_version_library scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for policy_version_library scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<PolicyVersionLibraryScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluatePolicyVersionLibraryScaffold(input: PolicyVersionLibraryScaffoldInput): PolicyVersionLibraryScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('policy_version_library scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('policy_version_library scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('policy_version_library scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<PolicyVersionLibraryScaffoldReceipt, 'scaffold_evidence_digest'> = {
    seed_id: PHASE_6C_POLICY_VERSION_LIBRARY_SEED_ID,
    component_id: PHASE_6C_POLICY_VERSION_LIBRARY_COMPONENT_ID,
    component_slug: "hr_performance_commission_policy_and_offboarding",
    model_name: "Phase6CPolicyVersionLibrary",
    event_name: POLICY_VERSION_LIBRARY_SCAFFOLD_EVENT,
