...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for at_risk_rules_engine scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for at_risk_rules_engine scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<AtRiskRulesEngineScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluateAtRiskRulesEngineScaffold(input: AtRiskRulesEngineScaffoldInput): AtRiskRulesEngineScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('at_risk_rules_engine scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('at_risk_rules_engine scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('at_risk_rules_engine scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<AtRiskRulesEngineScaffoldReceipt, 'scaffold_evidence_digest'> = {
    seed_id: PHASE_6C_AT_RISK_RULES_ENGINE_SEED_ID,
    component_id: PHASE_6C_AT_RISK_RULES_ENGINE_COMPONENT_ID,
    component_slug: "hr_performance_commission_policy_and_offboarding",
    model_name: "Phase6CAtRiskRulesEngine",
    event_name: AT_RISK_RULES_ENGINE_SCAFFOLD_EVENT,
