...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for mandatory_notice_classification scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for mandatory_notice_classification scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<MandatoryNoticeClassificationScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluateMandatoryNoticeClassificationScaffold(input: MandatoryNoticeClassificationScaffoldInput): MandatoryNoticeClassificationScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('mandatory_notice_classification scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('mandatory_notice_classification scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('mandatory_notice_classification scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<MandatoryNoticeClassificationScaffoldReceipt, 'scaffold_evidence_digest'> = {
    ...
    component_slug: "workspace_calendar_meetings_rooms_announcements",
    model_name: "Phase6CMandatoryNoticeClassification",
    event_name: MANDATORY_NOTICE_CLASSIFICATION_SCAFFOLD_EVENT,
