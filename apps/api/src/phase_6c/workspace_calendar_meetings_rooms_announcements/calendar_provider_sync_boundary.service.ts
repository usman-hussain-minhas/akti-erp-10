...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for calendar_provider_sync_boundary scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for calendar_provider_sync_boundary scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<CalendarProviderSyncBoundaryScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluateCalendarProviderSyncBoundaryScaffold(input: CalendarProviderSyncBoundaryScaffoldInput): CalendarProviderSyncBoundaryScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('calendar_provider_sync_boundary scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('calendar_provider_sync_boundary scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('calendar_provider_sync_boundary scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<CalendarProviderSyncBoundaryScaffoldReceipt, 'scaffold_evidence_digest'> = {
    ...
    component_slug: "workspace_calendar_meetings_rooms_announcements",
    model_name: "Phase6CCalendarProviderSyncBoundary",
    event_name: CALENDAR_PROVIDER_SYNC_BOUNDARY_SCAFFOLD_EVENT,
