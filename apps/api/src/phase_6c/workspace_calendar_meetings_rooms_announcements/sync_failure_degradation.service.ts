...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for sync_failure_degradation scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for sync_failure_degradation scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<SyncFailureDegradationScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluateSyncFailureDegradationScaffold(input: SyncFailureDegradationScaffoldInput): SyncFailureDegradationScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('sync_failure_degradation scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('sync_failure_degradation scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('sync_failure_degradation scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<SyncFailureDegradationScaffoldReceipt, 'scaffold_evidence_digest'> = {
    seed_id: PHASE_6C_SYNC_FAILURE_DEGRADATION_SEED_ID,
    component_id: PHASE_6C_SYNC_FAILURE_DEGRADATION_COMPONENT_ID,
    component_slug: "workspace_calendar_meetings_rooms_announcements",
    model_name: "Phase6CSyncFailureDegradation",
    event_name: SYNC_FAILURE_DEGRADATION_SCAFFOLD_EVENT,
