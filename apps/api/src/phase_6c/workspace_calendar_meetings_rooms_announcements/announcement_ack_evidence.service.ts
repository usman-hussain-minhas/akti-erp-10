...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for announcement_ack_evidence scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for announcement_ack_evidence scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<AnnouncementAckEvidenceScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluateAnnouncementAckEvidenceScaffold(input: AnnouncementAckEvidenceScaffoldInput): AnnouncementAckEvidenceScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('announcement_ack_evidence scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('announcement_ack_evidence scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('announcement_ack_evidence scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<AnnouncementAckEvidenceScaffoldReceipt, 'scaffold_evidence_digest'> = {
    seed_id: PHASE_6C_ANNOUNCEMENT_ACK_EVIDENCE_SEED_ID,
    component_id: PHASE_6C_ANNOUNCEMENT_ACK_EVIDENCE_COMPONENT_ID,
    component_slug: "workspace_calendar_meetings_rooms_announcements",
    model_name: "Phase6CAnnouncementAckEvidence",
    event_name: ANNOUNCEMENT_ACK_EVIDENCE_SCAFFOLD_EVENT,
