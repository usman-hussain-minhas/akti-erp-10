...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for waitlist_auto_promotion_timer scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for waitlist_auto_promotion_timer scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<WaitlistAutoPromotionTimerScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluateWaitlistAutoPromotionTimerScaffold(input: WaitlistAutoPromotionTimerScaffoldInput): WaitlistAutoPromotionTimerScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('waitlist_auto_promotion_timer scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('waitlist_auto_promotion_timer scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('waitlist_auto_promotion_timer scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<WaitlistAutoPromotionTimerScaffoldReceipt, 'scaffold_evidence_digest'> = {
    seed_id: PHASE_6C_WAITLIST_AUTO_PROMOTION_TIMER_SEED_ID,
    component_id: PHASE_6C_WAITLIST_AUTO_PROMOTION_TIMER_COMPONENT_ID,
    component_slug: "events_configuration_and_registration_service",
    model_name: "Phase6CWaitlistAutoPromotionTimer",
    event_name: WAITLIST_AUTO_PROMOTION_TIMER_SCAFFOLD_EVENT,
    ...
    business_behavior_allowed: false,
    runtime_adapter_allowed: false,
    decision_refs: ["6C-EVENT-REG-006"],
