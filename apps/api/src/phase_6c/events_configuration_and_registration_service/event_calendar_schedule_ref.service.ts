...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for event_calendar_schedule_ref scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for event_calendar_schedule_ref scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<EventCalendarScheduleRefScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluateEventCalendarScheduleRefScaffold(input: EventCalendarScheduleRefScaffoldInput): EventCalendarScheduleRefScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('event_calendar_schedule_ref scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('event_calendar_schedule_ref scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('event_calendar_schedule_ref scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<EventCalendarScheduleRefScaffoldReceipt, 'scaffold_evidence_digest'> = {
    seed_id: PHASE_6C_EVENT_CALENDAR_SCHEDULE_REF_SEED_ID,
    component_id: PHASE_6C_EVENT_CALENDAR_SCHEDULE_REF_COMPONENT_ID,
    component_slug: "events_configuration_and_registration_service",
    model_name: "Phase6CEventCalendarScheduleRef",
    event_name: EVENT_CALENDAR_SCHEDULE_REF_SCAFFOLD_EVENT,
    ...
    business_behavior_allowed: false,
    runtime_adapter_allowed: false,
    decision_refs: ["6C-EVENT-REG-009"],
