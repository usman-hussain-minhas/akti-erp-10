...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for ticket_type_capacity scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for ticket_type_capacity scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<TicketTypeCapacityScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluateTicketTypeCapacityScaffold(input: TicketTypeCapacityScaffoldInput): TicketTypeCapacityScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('ticket_type_capacity scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('ticket_type_capacity scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('ticket_type_capacity scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<TicketTypeCapacityScaffoldReceipt, 'scaffold_evidence_digest'> = {
    seed_id: PHASE_6C_TICKET_TYPE_CAPACITY_SEED_ID,
    component_id: PHASE_6C_TICKET_TYPE_CAPACITY_COMPONENT_ID,
    component_slug: "events_configuration_and_registration_service",
    model_name: "Phase6CTicketTypeCapacity",
    event_name: TICKET_TYPE_CAPACITY_SCAFFOLD_EVENT,
    ...
    business_behavior_allowed: false,
    runtime_adapter_allowed: false,
    decision_refs: ["6C-EVENT-REG-016"],
