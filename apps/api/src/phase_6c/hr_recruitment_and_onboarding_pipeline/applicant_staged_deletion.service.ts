...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for applicant_staged_deletion scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for applicant_staged_deletion scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<ApplicantStagedDeletionScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluateApplicantStagedDeletionScaffold(input: ApplicantStagedDeletionScaffoldInput): ApplicantStagedDeletionScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('applicant_staged_deletion scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('applicant_staged_deletion scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('applicant_staged_deletion scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<ApplicantStagedDeletionScaffoldReceipt, 'scaffold_evidence_digest'> = {
    seed_id: PHASE_6C_APPLICANT_STAGED_DELETION_SEED_ID,
    component_id: PHASE_6C_APPLICANT_STAGED_DELETION_COMPONENT_ID,
    component_slug: "hr_recruitment_and_onboarding_pipeline",
    model_name: "Phase6CApplicantStagedDeletion",
    event_name: APPLICANT_STAGED_DELETION_SCAFFOLD_EVENT,
