...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for offer_acceptance_employee_creation_request scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for offer_acceptance_employee_creation_request scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<OfferAcceptanceEmployeeCreationRequestScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluateOfferAcceptanceEmployeeCreationRequestScaffold(input: OfferAcceptanceEmployeeCreationRequestScaffoldInput): OfferAcceptanceEmployeeCreationRequestScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('offer_acceptance_employee_creation_request scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('offer_acceptance_employee_creation_request scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('offer_acceptance_employee_creation_request scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<OfferAcceptanceEmployeeCreationRequestScaffoldReceipt, 'scaffold_evidence_digest'> = {
    ...
    component_slug: "hr_recruitment_and_onboarding_pipeline",
    model_name: "Phase6COfferAcceptanceEmployeeCreationRequest",
    event_name: OFFER_ACCEPTANCE_EMPLOYEE_CREATION_REQUEST_SCAFFOLD_EVENT,
