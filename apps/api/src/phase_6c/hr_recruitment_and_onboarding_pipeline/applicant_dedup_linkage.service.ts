...
function requireNonEmpty(value: string | undefined, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(field + ' is required for applicant_dedup_linkage scaffold control.');
  }
  return value.trim();
}
...
function requireTimestamp(value: string | undefined, field: string): string {
  ...
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(field + ' must be a valid ISO-compatible timestamp for applicant_dedup_linkage scaffold control.');
  }
  return normalized;
}

function digestScaffold(receiptWithoutDigest: Omit<ApplicantDedupLinkageScaffoldReceipt, 'scaffold_evidence_digest'>): string {
  return createHash('sha256').update(JSON.stringify(receiptWithoutDigest)).digest('hex');
}

export function evaluateApplicantDedupLinkageScaffold(input: ApplicantDedupLinkageScaffoldInput): ApplicantDedupLinkageScaffoldReceipt {
  if (input.capability_execution_requested === true) {
    throw new Error('applicant_dedup_linkage scaffold control must not execute capability behavior.');
  }
  if (input.business_behavior_requested === true) {
    throw new Error('applicant_dedup_linkage scaffold control must not execute business behavior.');
  }
  if (input.runtime_adapter_requested === true) {
    throw new Error('applicant_dedup_linkage scaffold control must not execute runtime adapter behavior.');
  }

  const receiptWithoutDigest: Omit<ApplicantDedupLinkageScaffoldReceipt, 'scaffold_evidence_digest'> = {
    seed_id: PHASE_6C_APPLICANT_DEDUP_LINKAGE_SEED_ID,
    component_id: PHASE_6C_APPLICANT_DEDUP_LINKAGE_COMPONENT_ID,
    component_slug: "hr_recruitment_and_onboarding_pipeline",
    model_name: "Phase6CApplicantDedupLinkage",
    event_name: APPLICANT_DEDUP_LINKAGE_SCAFFOLD_EVENT,
