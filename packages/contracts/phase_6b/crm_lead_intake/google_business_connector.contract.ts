export const PHASE_6B_GOOGLE_BUSINESS_CONNECTOR_SEED_ID = 'seed_6b_04_google_business_connector' as const;
export const PHASE_6B_GOOGLE_BUSINESS_CONNECTOR_COMPONENT_ID = '6B.04' as const;

export const GOOGLE_BUSINESS_CONNECTOR_EVENT = 'phase_6b.crm_lead_intake.google_business_connector.intake_recorded' as const;

export type LeadOptOutObservation = 'NOT_OBSERVED' | 'OBSERVED_OPTED_OUT' | 'OBSERVED_OPTED_IN';

export type GoogleBusinessConnectorInput = {
  ...
  google_business_inquiry_id: string;
...
export type GoogleBusinessConnectorReceipt = {
  seed_id: typeof PHASE_6B_GOOGLE_BUSINESS_CONNECTOR_SEED_ID;
  component_id: typeof PHASE_6B_GOOGLE_BUSINESS_CONNECTOR_COMPONENT_ID;
  event_name: typeof GOOGLE_BUSINESS_CONNECTOR_EVENT;
  organization_id: string;
  service_manifest_contract_id: string;
  lead_record_id: string;
  person_identity_graph_id: string;
...
function requireNonEmpty(value: string, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(`${field} is required for Google Business connector.`);
  }
  return value.trim();
}
...
function normalizeOptOutObservation(value: LeadOptOutObservation | undefined): LeadOptOutObservation {
  ...
  if (!OPT_OUT_OBSERVATIONS.includes(value)) {
    throw new Error('opt_out_observation is not supported for Google Business connector.');
  }
  return value;
}
...
function requireCapturedAt(value: string): string {
  ...
  if (!Number.isFinite(timestamp)) {
    throw new Error('captured_at must be a valid ISO-compatible timestamp for Google Business connector.');
  }
  return normalized;
}
...
function normalizeFields(fields: Record<string, string>): Readonly<Record<string, string>> {
  if (fields === null || typeof fields !== 'object' || Array.isArray(fields)) {
    throw new Error('field_values must be an object for Google Business connector.');
  }
  ...
  if (Object.keys(normalized).length === 0) {
    throw new Error('field_values must contain at least one mapped Google Business inquiry field.');
  }
  return Object.freeze(normalized);
}

export function recordGoogleBusinessConnectorIntake(input: GoogleBusinessConnectorInput): GoogleBusinessConnectorReceipt {
  if (input.direct_provider_messaging_requested === true) {
    throw new Error('Google Business connector must not perform direct provider messaging.');
  }
  if (input.provider_callback_processing_requested === true) {
    throw new Error('Google Business connector FFET does not process provider callbacks.');
  }
  if (input.credential_material_included === true) {
    throw new Error('Google Business connector must reference API-key scope registry entries, not credential material.');
  }
  ...
  return {
    seed_id: PHASE_6B_GOOGLE_BUSINESS_CONNECTOR_SEED_ID,
    component_id: PHASE_6B_GOOGLE_BUSINESS_CONNECTOR_COMPONENT_ID,
    event_name: GOOGLE_BUSINESS_CONNECTOR_EVENT,
    ...
    source_system: 'GOOGLE_BUSINESS',
    google_business_profile_id: requireNonEmpty(input.google_business_profile_id, 'google_business_profile_id'),
    google_business_location_id: requireNonEmpty(input.google_business_location_id, 'google_business_location_id'),
    google_business_inquiry_id: requireNonEmpty(input.google_business_inquiry_id, 'google_business_inquiry_id'),
    source_interaction_ref: requireNonEmpty(input.source_interaction_ref, 'source_interaction_ref'),
