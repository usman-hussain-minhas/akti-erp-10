# P3-007B Validation Summary

Validation commands run:

```bash
pnpm --filter @akti/api test
pnpm --filter @akti/api typecheck
pnpm contracts:validate
pnpm exec prisma validate --schema prisma/schema.prisma
pnpm exec prisma generate --schema prisma/schema.prisma
pnpm registry:generate
git diff --exit-code -- generated/entity-registry.generated.json
pnpm registry:check
pnpm registry:verify:phase2
pnpm lint
pnpm typecheck
pnpm test
pnpm build
git diff -- prisma/schema.prisma
git diff -- prisma/entity-registry.metadata.json
git diff --check
git status --short --branch
```

Result:

- All validation commands passed after one bounded repair.
- Repair attempt 1 updated a brittle static assertion to tolerate formatting while preserving the trusted-context requirement.
- Generated registry had no drift.
- Prisma schema and registry metadata were unchanged.
- Controller source no longer reads `@Headers('x-actor-user-id')`.
