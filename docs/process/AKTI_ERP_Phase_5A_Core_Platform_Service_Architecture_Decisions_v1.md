...
Create the execution-grade Phase 5A service architecture decision for Tenant Configuration & Branding Service Architecture without drifting into Phase 5B runtime implementation.

### Decision / Policy

Tenant configuration and branding service resolves tenant config, white-label mode, domains, logos, color/label overrides, and visibility while preserving canonical internal identity.

### Required Rules

- Tenant-facing display may vary; canonical audit/source keys do not.
- Domain/branding changes require approval according to risk.
- Service architecture is defined only, not implemented in Phase 5A.
...
- Implement tenant config/branding service.
- Gatekeeper validates high-risk branding/domain changes.
- Frontend consumes resolved config.
