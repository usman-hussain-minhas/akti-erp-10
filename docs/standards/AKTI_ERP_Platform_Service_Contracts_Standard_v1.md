...
Create the execution-grade Phase 5A service/API contract standard for File / Document Service Contract without drifting into Phase 5B runtime implementation.

### Decision / Policy

File/document service contract defines metadata, upload/download intent, access checks, storage key abstraction, retention, audit, and safe error behavior.

### Required Rules

- Contracts never expose storage credentials.
- Files are tenant-scoped and classified.
- Runtime storage implementation is future work.
...
- Implement file service boundary.
- Gatekeeper validates file/document risk.
- Modules consume file references.
...
Create the execution-grade Phase 5A service/API contract standard for Reporting / Read-Model Query Contract without drifting into Phase 5B runtime implementation.

### Decision / Policy

Reporting/read-model query contract defines query inputs, filters, tenant boundary, freshness, source read model, paging, export behavior, and no-fake-data rule.

### Required Rules

- Queries use approved read models.
- Contract declares privacy class and capability gates.
- Reporting engine runtime is future work.
...
- Implement read-model query service.
- Gatekeeper validates report additions.
- Frontend dashboard/reporting uses real data only.
...
Create the execution-grade Phase 5A service/API contract standard for Data Import / Export Service Contract without drifting into Phase 5B runtime implementation.

### Decision / Policy

Data import/export service contract defines preview, validation, mapping, execution, error reporting, audit, tenant scope, and compensation behavior.

### Required Rules

- Imports/exports are tenant-scoped and capability-gated.
- Contracts do not grant runtime implementation in Phase 5A.
- Large or risky data movement requires approval.
...
- Implement service after policy approval.
- Gatekeeper validates data movement.
- Modules declare import/export compatibility.
...
Create the execution-grade Phase 5A service/API contract standard for AI Proxy Call Contract without drifting into Phase 5B runtime implementation.

### Decision / Policy

AI proxy call contract defines request purpose, model policy, data classes, capability gates, human review, budget/cost controls, audit, evaluation, and redaction requirements.

### Required Rules

- No runtime AI implementation is authorized in Phase 5A.
- Modules cannot bypass tenant/access/Gatekeeper through AI calls.
- AI outputs require review and evidence policy.
...
- Preserve contract for future governed AI service.
- Gatekeeper classifies AI requests.
- Module manifests declare AI readiness.
